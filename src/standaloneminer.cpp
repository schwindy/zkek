...
void test_mine(int n, int k, uint32_t d)
{
    ...
    while(true)
    {
        ...
        while(true)
        {
            ...
            (
                "pow",
                "Running Equihash solver with nNonce = %s\n",
                pblock.nNonce.ToString()
            );

            std::function<bool(std::vector<unsigned char>)>
                validBlock = [&pblock, &hashTarget, &nStart, &start_cycles](std::vector<unsigned char> soln)
                {
            ...
            std::function<bool(EhSolverCancelCheck)>
                cancelled = [](EhSolverCancelCheck pos)
                {
                    return false;
                };
...
int main(int argc, char* argv[])
{
    ...
    {
        std::string strUsage = _("Zcash Miner")+" "+ _("version")+" "+FormatFullVersion()+"\n";

        if(mapArgs.count("-version"))strUsage += LicenseInfo();
    ...
    if(init_and_check_sodium() == -1)return 1;

    LogPrintf("\n\n\n\n\n\n\n");
    LogPrintf("Zkek version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if(!stratum.empty() || GetBoolArg("-stratum", false))
    {
        ...
        while(sc.isRunning())MilliSleep(1000);
    }
    else
    {
