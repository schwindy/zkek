...
template<size_t WIDTH>
template<size_t W>
FullStepRow<WIDTH>::FullStepRow(const FullStepRow <W> &a, const FullStepRow <W> &b, size_t len, size_t lenIndices,
                                int trim) :
        StepRow<WIDTH>{a}
{
...
template<size_t WIDTH>
FullStepRow <WIDTH>
&FullStepRow<WIDTH>::operator=(const FullStepRow <WIDTH> &a)
{
...
IsZero(size_t len)
{
    ...
    for (int i = 0; i < len; i++)if (hash[i] != 0)return false;
    return true;
}

template<size_t WIDTH>
std::vector<unsigned char> FullStepRow<WIDTH>::
GetIndices(size_t len, size_t lenIndices, size_t cBitLen) const
{
    assert(((cBitLen + 1) + 7) / 8 <= sizeof(eh_index));
    size_t minLen{(cBitLen + 1) * lenIndices / (8 * sizeof(eh_index))};
    size_t bytePad{sizeof(eh_index) - ((cBitLen + 1) + 7) / 8};
...
{
    // This doesn't need to be constant time.
    for (int j = 0; j < l; j++)if (a.hash[j] != b.hash[j])return false;
    return true;
}

template<size_t WIDTH>
TruncatedStepRow<WIDTH>::TruncatedStepRow(const unsigned char *hashIn, size_t hInLen, size_t hLen, size_t cBitLen,
                                ...
                                const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    std::vector<FullStepRow < FullWidth>>
    X;
    X.reserve(init_size);
...
template<size_t WIDTH>
void CollideBranches(std::vector<FullStepRow < WIDTH>>
...
m++) {
if (
DistinctIndices(X[i + l], X[i + m], hlen, lenIndices
)) {
if (
IsValidBranch(X[i + l], hlen, ilen, lt
) &&
IsValidBranch(X[i + m], hlen, ilen, rt
)) {
Xc.
emplace_back(X[i + l], X[i + m], hlen, lenIndices, clen
);
} else if (
IsValidBranch(X[i + m], hlen, ilen, lt
) &&
IsValidBranch(X[i + l], hlen, ilen, rt
)) {
Xc.
emplace_back(X[i + m], X[i + l], hlen, lenIndices, clen
);
}
}
}
                                    ...
                                    const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    int invalidCount = 0;
    {
        ...
        std::vector<TruncatedStepRow < TruncatedWidth>>
        Xt;
                    ...
                    for (int m = l + 1; m < j; m++)
                    {
                        ...
                        if (!(Xi.IsZero(hashLen - CollisionByteLength) &&
                              IsProbablyDuplicate<soln_size>(
                                      Xi.GetTruncatedIndices(hashLen - CollisionByteLength, 2 * lenIndices),
                                      2 * lenIndices)))
                        {
                            Xc.emplace_back(Xi);
                        }
    ...
    for (std::shared_ptr<eh_trunc> partialSoln : partialSolns)
    {
        ...
        for (eh_index i = 0; i < soln_size; i++)
        {
            ...
            std::vector<FullStepRow < FinalFullWidth>>
            icv;
    ...
    LogPrint("pow", "Invalid solutions found: %d\n", invalidCount);
